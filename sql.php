...
function AudiorCommentsInstall(){
	...
	$simpleQueryResults = $wpdb->get_results( "DESCRIBE `".$settingsTable."`" );

	if ($simpleQueryResults) {
		//if settingsTable exists don't do anything
	} else {
		//if it doesn't, create the settings , comments and permissions tables 
		...
		//dbDelta is a WordPress function "useful for creating new tables and updating existing tables to a new structure."
		dbDelta($sql);
			
		$insert = "INSERT INTO ".$settingsTable." (license_key, file_name_prefix, show_sound_wave, marker_distance, language_file, disable_record_again_button, bg_corner_radius, 
