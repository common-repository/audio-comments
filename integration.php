<?php
/**
 * This file is used by Audior to load the settings exposed by the Audio Comments Plugin in WP's backend. Audio looks for this file before using the default settings. If this file does not exist Audior uses the default settings for all variables.
 *
 * @category	Comments
 * @package		Audio Comments Plugin for Wordpress
 * @author		NuSoft <contact@nusofthq.com>
 * @copyright	2017 audior.ec
...
//we start the session to populate the $_SESSION PHP variable
...
//canDownload  controls whether or not the user can download the MP3. Values: 0 for disabled, 1 for enabled.
$canDownload = 0;


/**
 * Audior settings that are controlled by the Audio Comments Plugin
 */

//recordName controls the name of the recording.
$recordName = $_SESSION['audior-settings']->file_name_prefix;

//whether or not the sound wave is shown, 1 for show, 0 for hidden.
...
/**
 * User level permissions that are controlled by the Audio Comments Plugin
 */
